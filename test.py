...
# ---------- Dataset (test only) ----------
test_raw = tfds.load("oxford_iiit_pet", split="test", shuffle_files=False)
...
def preprocess(image, mask):
    ...
    mask = tf.cast(mask, tf.int32) - 1  # 0..2
...
if not os.path.exists(MODEL_PATH):
    raise FileNotFoundError(
        ...
        f"python train.py --run_name {RUN_NAME} --num_classes {NUM_CLASSES}"
    )
...
# (safe) re-compile to ensure explicit metrics in evaluate output
model.compile(
    optimizer=tf.keras.optimizers.Adam(1e-3),
    loss=tf.keras.losses.SparseCategoricalCrossentropy(),
    metrics=[mean_iou, dice_coef],
