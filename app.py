...
# ======================
IMG_SIZE = 128
NUM_CLASSES = st.selectbox("Select number of classes", [2, 3])  # اضافه کردن انتخاب تعداد کلاس‌ها
MODEL_PATH = os.path.join("outputs", "best_unet.keras")
...
if uploaded is not None:
    ...
    with colB:
        st.write("Mask value meanings (trained on Oxford-IIIT Pet):")
        st.write(f"- 0: background")
        st.write(f"- 1: pet")
        if NUM_CLASSES == 3:
            st.write(f"- 2: border")
        else:
            st.write(f"- border is merged into 'pet'")
...
#اینو امتحان کنم و بعدش توی گیتهاب پوش کنم البته قبلش دو کلاسه رو مستنداتش رو اضافه کنم
#git add app.py
# git commit -m "feat: add dynamic class selection (2-class or 3-class)"
# git push
